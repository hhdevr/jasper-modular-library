...
3. Поля-субрепорты рекурсивно компилируются и заполняются, добавляя `<prefix>Report` и
   `<prefix>MapParameter`
4. Поля-коллекции кладутся в карту параметров как значения `JRBeanCollectionDataSource` —
   это **параметры**, а не корневой источник данных. В JRXML ссылайтесь на них через
   `$P{fieldName}` в `<dataSourceExpression>` компонента `list` или `table`
5. Вызывается `JasperFillManager.fillReport()` с картой параметров и `JREmptyDataSource`
   в качестве корневого источника данных — библиотека не использует band-iteration источники
