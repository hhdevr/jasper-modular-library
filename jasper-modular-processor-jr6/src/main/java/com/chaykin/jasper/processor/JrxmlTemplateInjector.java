 ...
 *
 * <p>This is the <strong>JasperReports 6.x</strong> implementation. The key difference
 * from the JR7 version is that {@link ComponentKey} must be set explicitly on every
 * component element. In JR6 {@code JRXmlWriter} uses {@code ComponentKey} to resolve the
 * XML namespace and element name during serialization — without it the writer throws
 * {@code "Error writing to OutputStream"}. In JR7 {@code ComponentKey} was removed and
 * the namespace is inferred automatically from the component class.</p>
 *
...
public class JrxmlTemplateInjector {
    ...
    /**
     * JasperReports components XML namespace URI.
     *
     * <p>Required by JR6 {@code JRXmlWriter} to correctly serialize list and table
     * component elements. Must be set via {@link ComponentKey} on each
     * {@link JRDesignComponentElement}.</p>
     */
    private static final String COMPONENTS_NAMESPACE =
            "http://jasperreports.sourceforge.net/jasperreports/components";

    private static final String COMPONENTS_NAMESPACE_PREFIX = "jr";
    private static final String LIST_COMPONENT_NAME = "list";
    ...
    /**
     * Creates a {@code list} component element.
     *
     * <p>Sets {@link ComponentKey} explicitly — required by JR6 {@code JRXmlWriter} to
     * emit the correct {@code <jr:list>} element with its namespace declaration.</p>
     */
    private JRDesignComponentElement createListComponent(JrxmlParameter field) {
        ...
        element.setComponentKey(new ComponentKey(COMPONENTS_NAMESPACE,
                                                 COMPONENTS_NAMESPACE_PREFIX,
                                                 LIST_COMPONENT_NAME));
    ...
    /**
     * Creates a {@code table} component element.
     *
     * <p>Sets {@link ComponentKey} explicitly — required by JR6 {@code JRXmlWriter} to
     * emit the correct {@code <jr:table>} element with its namespace declaration.</p>
     */
