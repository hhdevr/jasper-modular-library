 ...
 *       template, pre-populated with all parameters, datasets, list/table components and
...
public class JrxmlGeneratorProcessor extends AbstractProcessor {
    ...
    private static final String BLANK_PORTRAIT = "Portrait.jrxml";

    private static final String BLANK_LANDSCAPE = "Landscape.jrxml";

    private static final String BLANK_SUB_PORTRAIT = "SubPortrait.jrxml";

    private static final String BLANK_SUB_LANDSCAPE = "SubLandscape.jrxml";
    ...
    private void generate(TypeElement classElement) throws Exception {
        ...
        try (InputStream source = switch (mode) {
            case CREATE -> loadDefaultTemplate(classElement);
            case INJECT -> loadExistingOrDefault(templatePath, classElement);
     ...
     *
     * <p>If {@link JasperCollection} is present on the field, the component type and
     * column width are read from the annotation. Otherwise the defaults are used:
     * {@link CollectionComponentType#LIST} and {@link JasperCollection#DEFAULT_COLUMN_WIDTH},
     * which preserves the original behaviour for fields without the annotation.</p>
     *
     ...
     * @param componentType the JRXML component type to generate for this dataset
    ...
    /**
     * Loads the built-in blank JRXML template for the given class.
     *
     * <p>The template is selected based on the class annotation type and
     * {@link #resolveOrientation(TypeElement)}. Classes annotated with
     * {@link JasperSubreport} use zero-margin templates optimised for embedding;
     * root reports use standard A4 templates with default margins.</p>
     *
     * <p>Throws immediately if the template resource is missing — this indicates a bug
     * in jasper-modular, not a user error.</p>
     *
     * @param classElement the annotated class
     * @return an input stream for the blank template
     * @throws IllegalStateException if the template resource is not found in the jar
     */
    private InputStream loadDefaultTemplate(TypeElement classElement) {
        ...
        String templateName = isSubreport
                              ? (isLandscape ? BLANK_SUB_LANDSCAPE : BLANK_SUB_PORTRAIT)
                              : (isLandscape ? BLANK_LANDSCAPE : BLANK_PORTRAIT);

        String path = "/default-empty-reports/" + templateName;

        InputStream stream = getClass().getResourceAsStream(path);
        if (stream == null) {
            messager.printMessage(Diagnostic.Kind.ERROR,
                                  "Default template not found: " + path
                                  + ". This is a bug in jasper-modular — please report it.");
            throw new IllegalStateException("Default template not found: " + path);
        }
        return stream;
    }

    /**
     * Attempts to load an existing JRXML resource from the annotation processor classpath.
     * Falls back to the blank template if the file is not found.
     *
     * <p>The resource is looked up via the processor's own classloader rather than
     * {@link javax.tools.StandardLocation#SOURCE_OUTPUT}. This is necessary because
     * {@code StandardLocation.SOURCE_OUTPUT} may be empty at annotation processing time —
     * Maven copies {@code src/main/resources} to {@code target/classes} before compilation
     * starts, but the filer's output directory is separate and may not yet contain the
     * file on the first run. Using the classloader ensures the existing template is found
     * regardless of the build phase ordering.</p>
     *
     * @param templatePath the classpath-relative path to look up, e.g. {@code /reports/invoice.jrxml}
     * @param classElement the annotated class used to select the blank template orientation
     *                     as a fallback when no existing file is found
     * @return an input stream for the existing template, or for the blank default template
     * if no existing file was found on the classpath
     */
    private InputStream loadExistingOrDefault(String templatePath, TypeElement classElement) {
        ...
        return loadDefaultTemplate(classElement);
    }

    /**
     * Resolves the {@link PageOrientation} from the class annotation.
     *
     * @param classElement the annotated class
     * @return the declared orientation; defaults to {@link PageOrientation#PORTRAIT}
     */
