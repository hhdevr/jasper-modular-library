...
class JrxmlTemplateInjectorTest {
    ...
    private static final String BLANK = "/reports/prefilled_test.jrxml";
    ...
    private InputStream blankTemplate() {
        InputStream stream = getClass().getResourceAsStream(BLANK);
        assertThat(stream).as("blank_test.jrxml not found in src/test/resources/" + BLANK)
                          .isNotNull();
        return stream;
    }
    ...
    private InputStream prefilledTemplate() {
        ...
        assertThat(stream).as("prefilled_test.jrxml not found src/test/resources/" + PREFILLED)
                          .isNotNull();
        return stream;
    }
    ...
    class ListComponents {
        ...
        void existingListComponent_isNotDuplicated() throws Exception {
            ...
            // then - injected note appears exactly once, skip note appears on second run
            assertThat(messager.noteCount("Injected list component: " + COLLECTION_PARAM_NAME))
                    .isEqualTo(1);
    ...
    class Idempotency {
        ...
        void injectTwice_doesNotGrowParameterCount() throws Exception {
            ...
            var params = scalarParam();
            byte[] firstResult = injectToBytes(blankTemplate(), params);
            int countAfterFirst = JRXmlLoader
                    .load(new ByteArrayInputStream(firstResult))
                    .getParametersList().size();

            // when
            byte[] secondResult = injectToBytes(
                    new ByteArrayInputStream(firstResult), params);
            int countAfterSecond = JRXmlLoader
                    .load(new ByteArrayInputStream(secondResult))
                    .getParametersList().size();
